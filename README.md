...
The program is for creating and visualizeing different perspectives of a scene from a single image. The methodology is based on creating
...
After the user selected the background points and the vanishing point, a box model is created where the user can move using the keyboard.
...
The program calculates the corners of the five planes and plots them on the original image. If the user is satisfied, the algorithm 
based on these points, calculates an estimation of projective transformation for each plane and the estimated depth of images based on the 
similar triangles method from [2]. Then the planes are cropped and the transformation is made. As the last step, the planes are put 
