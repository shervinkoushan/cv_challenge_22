...
%This function builds the 3D dice,  by taking the five different side
%planes and warping the wanted plane pivture on it. As a bonus also a flat
%representation of the dice is created. 
%% Flat representation of 3D dice
...
%Placing the given image at the correct spots
...
%The size of the back image is used to generate the plane the picture is
%placed on.
[y_b, x_b, ~] = size(back);
[back_x,back_y] = meshgrid(1:x_b,1:y_b);
%Multiplication by 2 is to push the image forward so that we aviod an edge
%in the dice.
...
%Puts the given plane image onto the given plane
